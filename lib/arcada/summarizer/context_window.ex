defmodule Arcada.Summarizer.ContextWindow do
  @moduledoc """
  ...
  Everything is app-config driven (`config :arcada, #{inspect(__MODULE__)}`)
  ...
  The DB `max_text_chars` setting and the `Arcada.Summarizer`
  `:max_text_chars` config still win as explicit overrides (see
  `Arcada.Admin.max_text_chars/1`); this only replaces the hard-coded
  fallback.
  """
  ...
  defp cfg(key, default),
    do: Keyword.get(Application.get_env(:arcada, __MODULE__, []), key, default)
end
