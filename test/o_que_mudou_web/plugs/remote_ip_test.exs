defmodule OQueMudouWeb.Plugs.RemoteIpTest do
  ...
  alias OQueMudouWeb.Plugs.RemoteIp, as: Plug

  @peer {172, 18, 0, 5}

  setup do
    prev = Application.get_env(:o_que_mudou, :remote_ip)

    on_exit(fn ->
      case prev do
        nil -> Application.delete_env(:o_que_mudou, :remote_ip)
        val -> Application.put_env(:o_que_mudou, :remote_ip, val)
      end
    end)
  ...
  describe "disabled (config nil)" do
    test "is a no-op, leaving the socket peer as remote_ip" do
      Application.delete_env(:o_que_mudou, :remote_ip)
  ...
  describe "x-forwarded-for strategy" do
    setup do
      Application.put_env(:o_que_mudou, :remote_ip,
        headers: ["x-forwarded-for"],
        proxies: OQueMudouWeb.RemoteIpProxies.default(),
        clients: []
      )
  ...
  describe "config change" do
    test "recompiles cached options when the config value changes" do
      Application.put_env(:o_que_mudou, :remote_ip, headers: ["x-real-ip"], proxies: [])
      ...
      Application.put_env(:o_que_mudou, :remote_ip, headers: ["x-forwarded-for"], proxies: [])
      conn = run(conn_from([{"x-real-ip", "203.0.113.1"}, {"x-forwarded-for", "198.51.100.9"}]))
