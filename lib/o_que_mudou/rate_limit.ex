defmodule OQueMudou.RateLimit do
  @moduledoc """
  ...
  `OQueMudouWeb.RegisterLive`). The limit is therefore a load valve and a gentle
  ...
  Limits are config-driven (`config :o_que_mudou, OQueMudou.RateLimit, ...`) so
  ...
  def limits_for(tier, opts \\ []) do
    configured = Application.get_env(:o_que_mudou, __MODULE__, [])
