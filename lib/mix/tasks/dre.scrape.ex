defmodule Mix.Tasks.Dre.Scrape do
  ...
  Historical backfill — enqueues throttled Oban jobs and returns immediately:
  ...
  Backfill jobs run at low priority behind the daily pipeline and yield the GPU
  to foreign work, so they fill idle time without starving daily scraping or
  other jobs on the card. Idempotent: safe to re-run.
  """
  use Mix.Task
  ...
  defp run_backfill(opts) do
    ...
    Mix.shell().info(
      "Enqueued #{length(results)} backfill day(s) from #{Date.to_iso8601(from)} " <>
        "to #{Date.to_iso8601(to)} (business days only, newest first)."
    )
  end
