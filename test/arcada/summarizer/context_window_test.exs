defmodule Arcada.Summarizer.ContextWindowTest do
  use ExUnit.Case, async: false

  alias Arcada.Summarizer.ContextWindow

  # Swap the app config for the duration of a test, restoring it after.
  defp put_cfg(kw) do
    prev = Application.get_env(:arcada, ContextWindow, [])
    Application.put_env(:arcada, ContextWindow, Keyword.merge(prev, kw))
    on_exit(fn -> Application.put_env(:arcada, ContextWindow, prev) end)
  end
